...
void ResourcesManager::testCreateResource()
{
	...
	//}
	//m_displayedResourcesListModel.onResourceListUpdated(l_resourcesToDisplay);
}

bool ResourcesManager::addResource(const QUrl& p_resourceUrl)
{
	auto [_, l_inserted] { m_resources.emplace(std::make_unique<Resource>(Resource{ .m_name = p_resourceUrl.fileName(), .m_resourceUrl = p_resourceUrl })) };
